...
function WikiDetalle() {
  ...
  const fetchWikiDatos = useCallback(async () => {
    try {
      const response = await fetch(`http://localhost:8000/wikis/${nombre}`);
  ...
  const fetchArticulosWiki = useCallback(async () => {
    try {
      const response = await fetch(`http://localhost:8000/wikis/${nombre}/articulos`);
      const data = await response.json();
  ...
  const fetchBusqueda = useCallback(async (term) => {
    try {
      const response = await fetch(`http://localhost:8000/wikis/${nombre}/articulos?terminoDeBusqueda=${term}`);
      const data = await response.json();
      if (Array.isArray(data)) {
